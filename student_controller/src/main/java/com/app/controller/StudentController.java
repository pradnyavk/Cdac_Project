...
public class StudentController {
	 ...
	 * addTeacher(@RequestBody Teacher teacher, @PathVariable String id){ Student s
	 * = null ; long id1 = Long.parseLong(id); try { s = dao.findStudentById(id1);
	 * s.addTeacher(teacher); }catch(RuntimeException e) { return new
	 * ResponseEntity<Student>(s, HttpStatus.NO_CONTENT); } return new
	 * ResponseEntity<Student>(s, HttpStatus.OK); }
	 * 
	 * @PutMapping("/{id}/course")
	 * 
	 * @CrossOrigin(origins = "http://localhost:4200") public ResponseEntity<?>
	 * addCourse(@RequestBody Course course, @PathVariable String id){ Student s =
	 * null; long id1 = Long.parseLong(id); try { s = dao.findStudentById(id1);
	 * s.addCourse(course); }catch(RuntimeException e) { return new
	 * ResponseEntity<Student>(s, HttpStatus.NO_CONTENT); } return new
	 * ResponseEntity<Student>(s, HttpStatus.OK);
	 * 
	 * }
	...
	@CrossOrigin(origins = "http://localhost:4200")
	public ResponseEntity<?> addTeacher(@PathVariable String id, @RequestBody TeacherCourse teachercourse){
		long id1 = Long.parseLong(id);
		Student s1 = null;
		try {
			s1 = dao.findStudentById(id1);
			StudentTeacher st = new StudentTeacher();
			st.setStatus(false);
			st.setTeacherCourse(teachercourse);
			s1.addStudentTeacher(st);
			dao.saveStudent(s1);
		} catch (RuntimeException e) {
			return new ResponseEntity<Student>(s1, HttpStatus.NO_CONTENT);
		}
